...
window.onload = function () {
    ...
    const submitPath = urlParams.get('submit');
    const outputPath = urlParams.get('output');
    const baseUrl1 = submitPath;
    const baseUrl2 = outputPath;
    console.log("读取成功")
...
function _initCornerstone() {
    ...
    // Image Loader
    const config = {
        webWorkerPath: `http://localhost:5000/static/assets/js/Pictures/cornerstone/cornerstoneWADOImageLoader.js`,
        taskConfiguration: {
            decodeTask: {
                codecsPath: `http://localhost:5000/static/assets/js/Pictures/cornerstone/cornerstoneWADOImageLoader.js`,
            },
        },
    };
